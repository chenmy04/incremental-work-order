...
复制 `assets/work-order-template.md`。每张单必须让一个只读仓库、不读聊天记录的执行者也能完成，至少包含：
目标与**裁决来源（`R-xxxx`）**；实测现状（带出处）；精确范围与写权白名单；依赖（写成**可判定条件**）；
**Stages 与提交边界**；Gates（断言 / **反例** / **缺席时行为**）；DoD 六件套；唯一终态码；**所属批次**。

### 3.5 投递（把单放进子树，不是放进主树）

① 只碰白名单；② 写前 `git -C <子树> status --porcelain -- <路径>` 为空；③ 在**子树自己的分支**上
`git add -- <显式路径>` 并提交（信息 `dispatch work order <N> (from <主树> @<sha>)`）。
...
3. 动作：主树 `git merge --no-ff <executor-branch>`；冲突只在接缝处解决，不顺手改语义；
